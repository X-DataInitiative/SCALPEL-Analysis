...
def distribution_by_age_bucket(figure, cohort):
    df = agg(cohort.subjects, frozenset(["ageBucket"]), "count").sort_values("ageBucket")
    ...
    ax = figure.gca()
    ax = sns.barplot(x=df["ageBucket"].values, y=df["count(1)"].values, ax=ax, palette=colors)
...
def distribution_by_gender_age_bucket(figure, cohort):
    ...
    df.ageBucket = df.ageBucket.map(lambda x: BUCKET_INTEGER_TO_STR[x])
    ax = figure.gca()
    ax = sns.barplot(x=df["ageBucket"].values, y=df["count(1)"].values, hue=df["gender"].values, ax=ax)
    ax.set_xticklabels(ax.get_xticklabels(), rotation=90)
